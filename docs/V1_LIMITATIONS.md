# Axonel Public v1.0 Operational Limitations & Boundaries
...
This document provides a transparent, comprehensive accounting of Axonel's operational assumptions, supported integrations, known failure modes, and boundaries for version 0.1.0.
...
| **`fake_agent`** | Deterministic Local Test Mock | **Fully Proven** | **Tier 1 (Test / CI)** | Internal test harness backend used for deterministic regression suites and CI verification. |
...
| **`codex`** | OpenAI Codex / Aider | **Scaffold Stub** | **Tier 3 (Stub / Experimental)** | Interface stub registered; not certified for live production execution in v0.1.0. |
...
> [!WARNING]
> Do NOT attempt to run production coding missions with `claude_code`, `codex`, or `opencode` in v0.1.0. Google Gemini CLI (`gemini_cli`) is currently the **only** external agent backend certified for autonomous execution in this release.
...
- **Single-Repository Scope:** Each mission is strictly confined to a single Git repository workspace. Multi-repository transactions (e.g. coordinated edits across microservices) are unsupported in v0.1.0.
